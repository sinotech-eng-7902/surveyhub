/* v1.91 list performance: search, filter and client pagination for large datasets. */
(function(){
  'use strict';
  ...
  window.setFormPageSizeV191=function(value){formPageSize=Number(value)||10;formPage=1;renderFormsTable()};
  window.setFormPageV191=function(value){formPage=Number(value)||1;renderFormsTable();$('formsPanel')?.scrollIntoView({behavior:'smooth',block:'start'})};
  window.renderFormsTable=function(){var sections=formsBySection();if(!sections.some(function(x){return x.key===activeFormSection}))activeFormSection=sections[0]?.key||'mine';var current=sections.find(function(x){return x.key===activeFormSection})||sections[0],keyword=String($('formSearchV191')?.value||'').trim().toLowerCase(),state=String($('formStateFilterV191')?.value||''),items=(current?.items||[]).filter(function(f){var hay=[f.title,creatorSearchText(f)].join(' ').toLowerCase();return(!keyword||hay.includes(keyword))&&(!state||stateOf(f)===state)}),pages=Math.max(1,Math.ceil(items.length/formPageSize));formPage=Math.min(formPage,pages);var start=(formPage-1)*formPageSize,visible=items.slice(start,start+formPageSize);formsTable.innerHTML='<div class="surveyTabs">'+sections.map(function(section){return '<button class="'+(section.key===activeFormSection?'active':'')+'" onclick="setFormSection(\''+attr(section.key)+'\');setFormPageV191(1)">'+esc(section.title)+' <span>'+section.items.length+'</span></button>'}).join('')+'</div><p class="surveyTabHint">'+esc(current?.hint||'')+'</p>'+table(['問卷','狀態','我的角色','建立者','問卷期間','填寫','操作'],visible.map(formRowHtml),items.length?'':'沒有符合條件的問卷');formPage=pager($('formPaginationV191'),formPage,items.length,formPageSize,'setFormPageV191')||1};
  window.setMemberPageV191=function(value){memberPage=Number(value)||1;renderMemberPanel()};
  var baseMember=window.renderMemberPanel;
  window.renderMemberPanel=function(){baseMember();var target=$('membersTable'),body=target?.querySelector('tbody');if(!body)return;var rows=[].slice.call(body.children),total=rows.length,pages=Math.max(1,Math.ceil(total/memberPageSize));memberPage=Math.min(memberPage,pages);rows.forEach(function(row,index){row.hidden=index<(memberPage-1)*memberPageSize||index>=memberPage*memberPageSize});var pagerNode=$('memberPaginationV191');if(!pagerNode){target.insertAdjacentHTML('afterend','<div id="memberPaginationV191" class="paginationV191"></div>');pagerNode=$('memberPaginationV191')}memberPage=pager(pagerNode,memberPage,total,memberPageSize,'setMemberPageV191')||1};
