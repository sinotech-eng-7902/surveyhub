...
var QUESTION_TYPES_V132=[['short','簡答'],['long','長文'],['single','單選'],['multiple','複選'],['dropdown','下拉選單'],['department','部門選單'],['linearScale','線性刻度'],['rating','星等評分'],['time','時間'],['datetime','日期與時間'],['matrixSingle','單選矩陣'],['matrixMultiple','複選矩陣'],['file','檔案上傳題'],['image','內容區塊']];
...
function renderQuestionEditor(){
  ...
  var html=draftQuestions.map(function(q,i){
    var isContent=q.type==='image',questionNumber=draftQuestions.slice(0,i+1).filter(function(item){return normalizeQuestion(item).type!=='image'}).length,titleField='<label>'+(isContent?'區塊標題（選填）':'題目名稱')+'<input value="'+attr(q.title)+'" oninput="updateQuestion('+i+',\'title\',this.value)"></label>',typeField='<label>類型<select onchange="updateQuestion('+i+',\'type\',this.value);renderQuestionEditor()">'+QUESTION_TYPES_V132.map(function(x){return '<option value="'+x[0]+'" '+(q.type===x[0]?'selected':'')+'>'+x[1]+'</option>'}).join('')+'</select></label>',requiredField=isContent?'':'<label>必填<select onchange="updateQuestion('+i+',\'required\',this.value===\'true\')"><option value="false" '+(!q.required?'selected':'')+'>否</option><option value="true" '+(q.required?'selected':'')+'>是</option></select></label>';
    return '<div class="questionEdit '+(isContent?'contentBlockEditorV175':'')+'" data-question-index="'+i+'" draggable="true" ondragstart="onQuestionDragStart(event,'+i+')" ondragover="onQuestionDragOver(event,'+i+')" ondragleave="onQuestionDragLeave(event)" ondrop="onQuestionDrop(event,'+i+')" ondragend="onQuestionDragEnd(event)"><div class="questionEditHeader"><button type="button" class="dragHandle" title="拖曳排序" aria-label="拖曳排序">⋮⋮</button><span class="questionNumber">'+(isContent?'內容區塊':'題目 '+questionNumber)+'</span>'+questionJumpSelectHtmlV140(i)+'</div><div class="questionGrid '+(isContent?'contentBlockMainGridV175':'')+'">'+titleField+typeField+requiredField+'</div>'+optionEditorHtml(q,i)+questionImageEditorHtml(q,i)+contentBlockFileEditorHtmlV175(q,i)+questionMoreBar(q,i)+questionExtraSettingsHtml(q,i)+'<div class="miniActions"><button class="btn" type="button" onclick="moveQuestion('+i+',-1)">上移</button><button class="btn" type="button" onclick="moveQuestion('+i+',1)">下移</button><button class="btn" type="button" onclick="copyQuestion('+i+')">'+(isContent?'複製區塊':'複製題目')+'</button><button class="btn danger" type="button" onclick="removeQuestion('+i+')">移除</button></div></div>';
  }).join('');
  questionEditor.innerHTML=html||'<div class="questionHelp">尚未建立題目，請按「新增題目」。</div>';
  questionEditor.insertAdjacentHTML('beforeend','<div class="questionAddBottom"><button class="btn primary" type="button" onclick="addQuestion()">新增題目</button><button class="btn" type="button" onclick="addQuestion(\'image\')">加入內容區塊</button></div>');
...
function enhanceQuestionEditorV143(){
  ...
  editor.querySelectorAll('.questionEdit').forEach(function(card){
    ...
    if(actions&&!actions.querySelector('.questionAddAfterV143')){
      actions.insertAdjacentHTML('afterbegin','<button class="btn questionAddAfterV143" type="button" onclick="addQuestionAfterV143('+index+')">下方新增題目</button><button class="btn contentAddAfterV175" type="button" onclick="addQuestionAfterV143('+index+',\'image\')">下方加入內容</button>');
    }
  });
...
if(removeQuestionV144Base){
  removeQuestion=async function(i){
    ...
    var ok=await confirmDialog(isContent?'確定移除此內容區塊？':'確定移除此題？',isContent?'移除內容區塊':'移除題目',true);
    if(!ok)return;
...
if(addQuestionAfterV143V144Base){
  addQuestionAfterV143=function(index,type){
    ...
    renderQuestionEditor();
    focusQuestionByIdV144(activeQuestionIdV144);
    toast(type==='image'?'已在下方加入內容區塊':'已在下方新增題目','success');
  };
}
