...
function updateIdentityMembers(department){let select=$('identityMember'),list=members.filter(m=>m.active!==false&&(m.department||m.departmentName||'')===department);select.innerHTML='<option value="">請選擇姓名</option>'+list.map(m=>{let employeeNo=memberEmployeeNo(m);return `<option value="${attr(m.id)}">${employeeNo?esc(employeeNo)+' ':''}${esc(m.name)}</option>`}).join('');select.disabled=!department}
...
function renderFormManagers(){$('formManagersTable').innerHTML=table(['部門','姓名','員工編號','權限','狀態','操作'],formManagers.map(m=>{let id=attr(m.id),enabled=m.enabled!==false,member=findMemberByGoogleEmail(m.email),department=String(member?.department||member?.departmentName||'').trim()||'未紀錄',name=String(member?.name||'').trim()||'未紀錄',employeeNo=String(member?.employeeNo||member?.empNo||'').trim()||'未紀錄';return `<tr><td>${esc(department)}</td><td><b>${esc(name)}</b></td><td>${esc(employeeNo)}</td><td>${esc(m.role==='manager'?'問卷管理者':'結果檢視者')}</td><td><span class="statePill ${enabled?'state-open':'state-closed'}">${enabled?'啟用':'停用'}</span></td><td>${actionGroup([actionButton('移除',`removeFormManager('${id}')`,'danger')])}</td></tr>`}),'尚無資料');$('formManagersTable').querySelector('table')?.classList.add('formPermissionTableV161')}
