...
function fileAcceptV154(q){var kind=normalizeQuestion(q).settings.fileKind;if(kind==='image')return'image/*';if(kind==='document')return'.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.zip,.rar,.7z';return''}
...
renderPublicQuestion=function(q,prefix,value){
  ...
  return '<section class="questionCard fileQuestionV154"><label class="title">'+esc(q.title||'未命名題目')+required+'</label>'+help+
    ...
    '<label class="fileDropFieldV154"><span class="fileDropIconV154" aria-hidden="true">＋</span><span><b>選擇'+(multiple?'檔案':'一個檔案')+'</b><small>每個檔案上限 10 MB'+(multiple?'，最多 '+limit+' 個':'')+'</small></span><input type="file" name="'+attr(name)+'" accept="'+attr(fileAcceptV154(q))+'" '+(multiple?'multiple':'')+' '+(q.required&&!existing.length?'required':'')+'></label></section>';
};
