...
function validateImageFile(file){if(!file)return'請先選擇圖片';if(!IMAGE_TYPES.includes(file.type))return'僅支援 JPG、PNG 或 WebP 圖片';if(file.size>IMAGE_MAX_BYTES)return'圖片檔案不可超過 10 MB';return''}
...
function answeredResponseCountV168(q){return responses.reduce(function(total,response){return total+(chartAnswerValuesV168(q,response.answers&&response.answers[q.id]).length?1:0)},0)}

function optionCounts(q){var map=new Map((Array.isArray(q.options)?q.options:[]).map(function(option){return [String(option),0]}));for(var response of responses){var values=chartAnswerValuesV168(q,response.answers&&response.answers[q.id]);for(var key of values)map.set(key,(map.get(key)||0)+1)}return [...map.entries()].map(function(entry){return {label:entry[0],count:entry[1]}})}

function pieHtml(title,items,denominator=responses.length){items=(Array.isArray(items)?items:[]).map(function(item){var count=Number(item&&item.count);return {label:String(item&&item.label!=null?item.label:'未命名選項'),count:Number.isFinite(count)&&count>0?Math.round(count):0}});var shown=items.map(function(x,index){return {label:x.label,count:x.count,index:index}}).filter(function(x){return x.count>0}),sum=shown.reduce(function(n,x){return n+x.count},0),cursor=0;var slices=shown.map(function(x){var start=cursor,end=cursor+(sum?x.count/sum*360:0),p=percentage(x.count,denominator),path=pieSlicePathV165(start,end);cursor=end;return '<path d="'+path+'" fill="'+chartColors[x.index%chartColors.length]+'" class="chartSliceV165 chartInteractiveV165" tabindex="0" data-chart-index="'+x.index+'" data-chart-label="'+attr(x.label)+'" data-chart-count="'+x.count+'" data-chart-percent="'+p+'" aria-label="選項 '+attr(x.label)+'，票數 '+x.count+'，百分比 '+p+'%"></path>'}).join(''),chart=sum?'<svg class="pieSvgV165" viewBox="0 0 128 128" role="img" aria-label="'+attr(title)+'圓餅圖">'+slices+'</svg>':'<div class="pieEmptyV165" aria-label="尚無資料"></div>',legend=items.map(function(x,i){var p=percentage(x.count,denominator);return '<div class="legendRow chartLegendInteractiveV165 chartInteractiveV165" tabindex="0" data-chart-index="'+i+'" data-chart-label="'+attr(x.label)+'" data-chart-count="'+x.count+'" data-chart-percent="'+p+'" aria-label="選項 '+attr(x.label)+'，票數 '+x.count+'，百分比 '+p+'%"><span class="legendDot" style="background:'+chartColors[i%chartColors.length]+'"></span><span>'+esc(x.label)+'</span><strong>'+x.count+' 人・'+p+'%</strong></div>'}).join('')||'<span class="questionHelp">尚無資料</span>';return '<div class="analysisCard"><h3>'+esc(title)+'</h3><div class="pieLayout"><div class="pieChart pieChartV165">'+chart+'</div><div class="chartLegend">'+legend+'</div></div></div>'}

function multipleAnalysisHtml(q){var items=optionCounts(q),total=answeredResponseCountV168(q);return '<div class="analysisCard"><h3>'+esc(q.title)+'</h3><div class="barList">'+(items.map(function(x){var p=percentage(x.count,total);return '<div'+chartDatumAttrsV165(x.label,x.count,p)+'><div class="barRowHead"><span>'+esc(x.label)+'</span><strong>'+x.count+' 人・'+p+'%</strong></div><div class="barTrack"><div class="barFill" style="width:'+p+'%"></div></div></div>'}).join('')||'<span class="questionHelp">尚無資料</span>')+'</div></div>'}
...
async function openMissingList(button){await showPanel('resultsPanel',button);let panel=ensureMissingPanel();if(panel)panel.scrollIntoView({behavior:'smooth',block:'start'})}
...
function scaleAnalysisHtml(q){var labels=scaleItems(q),valid=new Set(labels),answered=responses.map(function(response){var raw=(response.answers||{})[q.id],text=String(raw==null?'':raw).trim();return text&&valid.has(text)?Number(text):null}).filter(function(value){return value!=null&&Number.isFinite(value)}),items=labels.map(function(label){return {label:label,count:answered.filter(function(value){return String(value)===label}).length}}),denominator=answered.length,avg=answered.length?Math.round(answered.reduce(function(a,b){return a+b},0)*10/answered.length)/10:null,summary=avg==null?'尚無評分':'平均 '+avg;return '<div class="analysisCard"><h3>'+esc(q.title)+' <small>'+summary+'</small></h3><div class="barList">'+items.map(function(x){var p=percentage(x.count,denominator);return '<div'+chartDatumAttrsV165(x.label,x.count,p)+'><div class="barRowHead"><span>'+esc(x.label)+'</span><strong>'+x.count+' 人・'+p+'%</strong></div><div class="barTrack"><div class="barFill" style="width:'+p+'%"></div></div></div>'}).join('')+'</div></div>'}
function matrixAnalysisHtml(q){var rows=Array.isArray(q.rows)?q.rows:[],cols=Array.isArray(q.columns)?q.columns:[];return '<div class="analysisCard wideAnalysis"><h3>'+esc(q.title)+'</h3><div class="matrixScroll"><table class="matrixTable"><thead><tr><th>列項目</th>'+cols.map(function(c){return '<th>'+esc(c)+'</th>'}).join('')+'</tr></thead><tbody>'+rows.map(function(row){var answered=responses.map(function(response){return response.answers&&response.answers[q.id]&&response.answers[q.id][row]}).filter(function(value){return Array.isArray(value)?value.some(function(item){return String(item==null?'':item).trim()}):String(value==null?'':value).trim()});return '<tr><th>'+esc(row)+'</th>'+cols.map(function(col){var count=answered.filter(function(value){return Array.isArray(value)?new Set(value.map(String)).has(String(col)):String(value)===String(col)}).length,p=percentage(count,answered.length);return '<td'+chartDatumAttrsV165(row+'－'+col,count,p)+'>'+count+'</td>'}).join('')+'</tr>'}).join('')+'</tbody></table></div></div>'}
...
function updateRoleUi(){ensureAdminExtensions();if($('membersNav'))$('membersNav').style.display=isSystemAdmin?'':'none';if($('permissionsNav'))$('permissionsNav').style.display=activeFormId&&canManageForm(activeFormId)?'':'none';if($('systemManagementNavGroup'))$('systemManagementNavGroup').style.display=(isSystemAdmin||(activeFormId&&canManageForm(activeFormId)))?'':'none';if(!isSystemAdmin&&$('membersPanel')&&$('membersPanel').classList.contains('active'))showPanel('dashboardPanel');if(activeFormId&&!canManageForm(activeFormId)&&(($('editorPanel')&&$('editorPanel').classList.contains('active'))||($('permissionsPanel')&&$('permissionsPanel').classList.contains('active'))))showPanel('resultsPanel')}
...
submitAssistedResponse=async function(event){
  ...
  try{
    ...
    await loadResponses();renderAdmin();showPanel('resultsPanel');
    var panel=ensureMissingPanel();if(panel)panel.scrollIntoView({behavior:'smooth',block:'start'});
