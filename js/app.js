...
function pieHtml(title, items, denominator) {
  ...
  return '<div class="analysisCard"><h3>' + esc(title) + ' <small style="color:var(--muted);font-size:13px;font-weight:normal;">（' + denominator + ' 份回答）</small></h3><div class="pieLayout" style="gap: 40px; padding: 20px 10px;"><div style="width:260px;height:260px;flex:0 0 260px;">' + pieChartHtml + '</div><div class="chartLegend">' + (items.map((x, i) => '<div class="legendRow" style="display:flex;align-items:center;gap:10px;font-size:14.5px;margin-bottom:8px;"><span class="legendDot" style="display:inline-block;flex:0 0 14px;width:14px;height:14px;border-radius:50%;margin:0;background:' + chartColors[i % chartColors.length] + '"></span><span style="margin-right:4px;">' + esc(x.label) + '</span><strong style="text-align:left;min-width:0;color:var(--muted);font-weight:normal;">' + x.count + ' 人・' + percentage(x.count, denominator) + '%</strong></div>').join('') || '<span class="questionHelp">尚無資料</span>') + '</div></div></div>';
}
