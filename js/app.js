...
const pendingContentBlockFilesV175=new Map();
...
function setLoginFeedbackV176(message=''){
  loginFeedbackV176=String(message||'');
  if(loginMsg)loginMsg.textContent=loginFeedbackV176;
  let homeMessage=$('homeLoginMsgV176');
  if(homeMessage)homeMessage.textContent=loginFeedbackV176;
}

function homeLoginHtmlV176(){
  return '<section class="homeLoginV176" aria-labelledby="homeLoginTitleV176">'+
    '<div class="homeLoginVisualV176" aria-hidden="true">'+
      '<div class="visualCardV176 visualCardMainV176"><span></span><i></i><i></i><i></i></div>'+
      '<div class="visualCardV176 visualCardChartV176"><b></b><b></b><b></b><b></b></div>'+
      '<div class="visualCardV176 visualCardRingV176"><span></span></div>'+
      '<div class="visualGlowV176"></div>'+
    '</div>'+
    '<div class="homeLoginCardV176">'+
      '<img src="assets/company-logo.png" alt="環興科技股份有限公司">'+
      '<div class="homeLoginDividerV176" aria-hidden="true"></div>'+
      '<h1 id="homeLoginTitleV176">公司通用問卷調查系統</h1>'+
      '<p>問卷管理與填寫結果整合平台</p>'+
      '<button class="homeGoogleButtonV176" type="button" onclick="startHomeAdminLoginV176(this)"><span class="googleMarkV176" aria-hidden="true">G</span><span>使用 Google 帳號登入</span></button>'+
      '<p id="homeLoginMsgV176" class="homeLoginMessageV176" aria-live="polite">'+esc(loginFeedbackV176)+'</p>'+
    '</div>'+
  '</section>';
}
...
function resetPendingImages(){revokeObjectUrl(pendingHeaderImagePreviewUrl);pendingHeaderImageFile=null;pendingHeaderImagePreviewUrl='';pendingQuestionImagePreviewUrls.forEach(revokeObjectUrl);pendingQuestionImageFiles.clear();pendingQuestionImagePreviewUrls.clear();resetPendingContentBlockFilesV175()}
...
async function init(){
  normalizeLegacyManageRouteV177();
  ...
  document.querySelectorAll('.topNavGroup').forEach(group=>{group.addEventListener('mouseenter',()=>{clearTimeout(window.__topNavCloseTimer);if(window.matchMedia('(hover:hover) and (pointer:fine)').matches){document.querySelectorAll('.topNavGroup[open]').forEach(other=>{if(other!==group)other.removeAttribute('open')});group.setAttribute('open','')}});group.addEventListener('mouseleave',()=>{if(!window.matchMedia('(hover:hover) and (pointer:fine)').matches)return;clearTimeout(window.__topNavCloseTimer);window.__topNavCloseTimer=setTimeout(()=>group.removeAttribute('open'),200)});group.querySelectorAll('.nav').forEach(item=>item.addEventListener('click',()=>closeTopNavGroups()))});
  auth.onAuthStateChanged(async user=>{stopResponseListenerV179();currentUser=user;isAdmin=false;isSystemAdmin=false;formAssignments=[];if(user){try{await loadMemberAccounts();isSystemAdmin=await checkAdmin(user);formAssignments=await loadCurrentAssignments(user);let member=findMemberByGoogleEmail(user.email),memberDisabled=member?.active===false,hasMemberAccount=!!member&&!memberDisabled;isAdmin=isSystemAdmin||(!memberDisabled&&(hasMemberAccount||formAssignments.some(x=>x.enabled!==false)));if(isAdmin){setLoginFeedbackV176('');let name=memberDisplayName(member)||user.displayName||'後台使用者';if($('adminUserName'))$('adminUserName').textContent=name;adminUser.textContent=isSystemAdmin?'系統管理員':'問卷管理者';loginMask.style.display='none';await loadAdminData()}else if(loginPurpose==='response'){loginMask.style.display='none'}else setLoginFeedbackV176(memberDisabled?'此人員帳號目前已停用，如有疑問請聯絡系統管理員。':'此 Google 帳號尚未建立於人員管理或問卷權限中，請聯絡系統管理員。')}catch(e){console.error('admin auth failed',e);setLoginFeedbackV176('後台登入檢查失敗，請確認 Firestore 規則與人員 Google 帳號設定。')}}else{memberAccounts=[]}initialAuthResolved=true;applyRoute()});
...
function openAdmin(){loginPurpose='admin';history.pushState(null,'','#manage');applyRoute()}
...
function startHomeAdminLoginV176(button){loginPurpose='admin';return loginGoogle(button)}
async function loginGoogle(triggerButton){let button=triggerButton||loginBtn;if(!auth){setLoginFeedbackV176('登入服務尚未完成初始化，請重新整理頁面後再試一次。');return}if(button){button.disabled=true;button.classList.add('isLoadingV176')}setLoginFeedbackV176('');try{let p=new firebase.auth.GoogleAuthProvider();p.setCustomParameters({prompt:'select_account'});await auth.signInWithPopup(p);if(loginPurpose==='admin'&&!isAdmin)setLoginFeedbackV176('正在確認管理員權限…')}catch(e){setLoginFeedbackV176(e.code==='auth/popup-closed-by-user'?'已取消登入。':(e.message||'登入失敗'))}finally{if(button){button.disabled=false;button.classList.remove('isLoadingV176')}}}
async function logout(){stopResponseListenerV179();activeFormId='';loginPurpose='admin';setLoginFeedbackV176('');await auth.signOut();history.replaceState(null,'',location.pathname+location.search);applyRoute()}
...
function effectiveState(form){if(form?.deleted)return'deleted';var state=form?.state||'draft';if(state==='open'&&deadlinePassed(form.deadline))return'expired';if(state==='open'&&form.startTime&&new Date(form.startTime).getTime()>Date.now())return'scheduled';return state}
function stateLabel(v){return({draft:'草稿',scheduled:'排程開放',open:'開放填寫',closed:'已關閉',deleted:'已移至垃圾桶',expired:'已截止',pending:'尚未開始'})[v]||v}
...
async function removeQuestion(i){var target=normalizeQuestion(draftQuestions[i]),isContent=target.type==='image';if(await confirmDialog(isContent?'確定移除此說明內容？':'確定移除此題？',isContent?'移除說明內容':'移除題目',true)){var q=draftQuestions[i];if(q){revokeObjectUrl(pendingQuestionImagePreviewUrls.get(q.id));pendingQuestionImageFiles.delete(q.id);pendingQuestionImagePreviewUrls.delete(q.id);pendingContentBlockFilesV175.delete(String(q.id))}draftQuestions.splice(i,1);markFormDirty();renderQuestionEditor()}}
...
/* v1.77: use neutral management URLs while keeping legacy admin bookmarks compatible. */
function normalizeLegacyManageRouteV177(){let hash=location.hash;if(hash==='#admin'||hash.startsWith('#admin/')){history.replaceState(null,'','#manage'+hash.slice(6));return true}return false}
function adminRouteId(){let m=location.hash.match(/^#manage\/([^/?#]+)/);return m?decodeURIComponent(m[1]):''}
...
function renderMemberPanel(){let target=$('membersTable');if(!target)return;let keyword=String($('memberSearch')?.value||'').trim().toLowerCase(),list=members.filter(m=>!keyword||`${m.department||m.departmentName||''} ${m.name||''} ${m.employeeNo||m.empNo||''} ${memberGoogleEmail(m)} ${memberCompanyEmail(m)}`.toLowerCase().includes(keyword));$('memberCountLabel').textContent=`顯示 ${list.length}／${members.length} 人`;target.innerHTML=table(['部門','姓名','員工編號','Google 帳號','公司信箱','狀態','操作'],list.map(m=>{let active=m.active!==false;return `<tr><td>${esc(m.department||m.departmentName||'')}</td><td><b>${esc(m.name||'')}</b></td><td>${esc(m.employeeNo||m.empNo||'')}</td><td>${memberGoogleEmail(m)?esc(memberGoogleEmail(m)):'<span class="questionHelp">未設定</span>'}</td><td>${memberCompanyEmail(m)?esc(memberCompanyEmail(m)):'<span class="questionHelp">未設定</span>'}</td><td><span class="statusBadge ${active?'active':'inactive'}">${active?'啟用':'停用'}</span></td><td><div class="buttonRow"><button class="btn" onclick="editMember('${attr(m.id)}')">編輯</button><button class="btn" onclick="toggleMember('${attr(m.id)}',${active?'false':'true'})">${active?'停用':'啟用'}</button><button class="btn danger" onclick="deleteMember('${attr(m.id)}')">刪除</button></div></td></tr>`}))}

function fillMemberEditor(m=null){$('memberDepartment').innerHTML='<option value="">請選擇部門</option>'+memberDepartmentOptions(m?.department||m?.departmentName||'');$('memberName').value=m?.name||'';$('memberEmployeeNo').value=m?.employeeNo||m?.empNo||'';$('memberGoogleEmail').value=m?memberGoogleEmail(m):'';if($('memberCompanyEmail'))$('memberCompanyEmail').value=m?memberCompanyEmail(m):'';$('memberActive').value=String(m?.active!==false)}
...
function downloadMemberTemplate(){XLSX.writeFile(memberWorkbook([{'部門':'行政部','姓名':'王小明','員工編號':'7901','Google帳號':'example@gmail.com','公司信箱':'example@mail.sinotech-eng.com','狀態':'啟用'}],'匯入範本'),'人員匯入標準範本.xlsx')}

function exportMembers(){let rows=members.map(m=>({'部門':m.department||m.departmentName||'','姓名':m.name||'','員工編號':m.employeeNo||m.empNo||'','Google帳號':memberGoogleEmail(m),'公司信箱':memberCompanyEmail(m),'狀態':m.active===false?'停用':'啟用'}));XLSX.writeFile(memberWorkbook(rows),'共用人員名單.xlsx')}
...
async function importMembersV152(file){
  ...
  try{
    ...
    let googleHeaders=['Google帳號','Google 帳號','Google Email','Email','電子郵件'],companyHeaders=['公司信箱','公司 Email','公司Email','工作信箱'],hasGoogleColumn=Object.keys(rows[0]||{}).some(key=>googleHeaders.includes(String(key).replace(/^\uFEFF/,'').trim())),hasCompanyColumn=Object.keys(rows[0]||{}).some(key=>companyHeaders.includes(String(key).replace(/^\uFEFF/,'').trim())),validDepartments=new Set(departments.map(d=>d.name||d.departmentName||d.department||'').filter(Boolean)),existingByNo=new Map(members.map(m=>[String(m.employeeNo||m.empNo||'').trim(),m]).filter(x=>x[0])),seen=new Set(),uploadedEmployeeNos=new Set(),seenEmails=new Set(),seenCompanyEmails=new Set(),errors=[],items=[];
    rows.forEach((row,index)=>{
      let line=index+2,department=String(memberCell(row,['部門'])).trim(),name=String(memberCell(row,['姓名'])).trim(),employeeNo=String(memberCell(row,['員工編號','員編'])).trim(),googleEmail=normalizeEmail(memberCell(row,['Google帳號','Google 帳號','Google Email','Email','電子郵件'])),companyEmail=normalizeEmail(memberCell(row,['公司信箱','公司 Email','公司Email','工作信箱'])),status=String(memberCell(row,['狀態'])).trim();
...
function stopResponseListenerV179(){
  responseListenerGenerationV179++;
  if(typeof responseUnsubscribeV179==='function'){
    try{responseUnsubscribeV179()}catch(e){console.warn('解除填寫資料監聽失敗',e)}
  }
  responseUnsubscribeV179=null;
  responseListenerFormIdV179='';
}
function responseSnapshotItemsV179(snapshot){
  return snapshot.docs.map(function(item){return{id:item.id,...item.data()}}).sort(function(a,b){return(b.submittedAt?.seconds||0)-(a.submittedAt?.seconds||0)});
}
function renderRealtimeResponseViewsV179(){
  var panel=document.querySelector('.panel.active'),panelId=panel&&panel.id,scrollX=window.scrollX,scrollY=window.scrollY;
  renderDashboard();
  if(panelId==='formsPanel')renderFormsTable();
  if(panelId==='resultsPanel')renderResults();
  if(panelId==='progressPanel')renderProgressPanelV171();
  requestAnimationFrame(function(){window.scrollTo(scrollX,scrollY)});
}
function startResponseListenerV179(formId=activeFormId){
  formId=String(formId||'');
  if(!isAdmin||!formId||!canViewForm(formId)){stopResponseListenerV179();return}
  if(responseUnsubscribeV179&&responseListenerFormIdV179===formId)return;
  stopResponseListenerV179();
  var generation=++responseListenerGenerationV179;
  responseListenerFormIdV179=formId;
  responseUnsubscribeV179=col('universalResponses').where('formId','==',formId).onSnapshot(function(snapshot){
    if(generation!==responseListenerGenerationV179||activeFormId!==formId)return;
    responses=responseSnapshotItemsV179(snapshot);
    renderRealtimeResponseViewsV179();
  },function(error){
    if(generation!==responseListenerGenerationV179)return;
    console.warn('填寫資料即時同步失敗，保留手動重新整理備援',error);
  });
}
async function loadAdminData(){await loadPublicData();await loadMemberAccounts();if(isSystemAdmin&&!submissionLocksPrepared)await prepareSubmissionLocks();await loadResponses();startResponseListenerV179(activeFormId);renderAdmin()}
...
async function loadResponses(requestedFormId=activeFormId){let formId=String(requestedFormId||''),requestId=++responseLoadRequestV178;if(!isAdmin||!formId||!canViewForm(formId)){if(requestId===responseLoadRequestV178&&activeFormId===formId)responses=[];return false}try{let q=await col('universalResponses').where('formId','==',formId).get();if(requestId!==responseLoadRequestV178||activeFormId!==formId)return false;responses=q.docs.map(x=>({id:x.id,...x.data()})).sort((a,b)=>(b.submittedAt?.seconds||0)-(a.submittedAt?.seconds||0));return true}catch(e){console.warn(e);if(requestId===responseLoadRequestV178&&activeFormId===formId)responses=[];return false}}
function applyRoute(){if(!syncInitialBootState()){front.style.display='none';admin.style.display='none';loginMask.style.display='none';return}normalizeLegacyManageRouteV177();let wantsAdmin=location.hash==='#manage'||location.hash.startsWith('#manage/');if((!location.hash||location.hash==='#')&&isAdmin){history.replaceState(null,'','#manage');wantsAdmin=true}document.body.classList.toggle('homeLoginActiveV176',!location.hash||location.hash==='#');if(wantsAdmin&&isAdmin){document.body.classList.remove('homeLoginActiveV176');front.style.display='none';admin.style.display='block';loginMask.style.display='none';let routeId=adminRouteId();if(routeId&&canViewForm(routeId)&&routeId!==activeFormId){stopResponseListenerV179();activeFormId=routeId;loadResponses().then(function(){startResponseListenerV179(routeId);renderAdmin()});return}startResponseListenerV179(activeFormId);renderAdmin();return}admin.style.display='none';front.style.display='block';loginMask.style.display=wantsAdmin&&!isAdmin?'grid':'none';renderFront();if(loginPurpose==='response'&&currentUser)setTimeout(showMyResponse,0)}
...
async function startMyResponseView(){loginPurpose='response';setLoginFeedbackV176('');if(!currentUser){loginMask.style.display='grid';return}await loadMemberAccounts();await showMyResponse()}
...
function renderAdmin(){ensureAdminExtensions();let list=accessibleForms();if(list.length&&!list.some(f=>f.id===activeFormId))activeFormId=list[0].id;activeFormSelect.innerHTML='<option value="">請選擇問卷</option>'+list.map(f=>`<option value="${attr(f.id)}" ${f.id===activeFormId?'selected':''}>${esc(f.title)}</option>`).join('');let f=activeForm();activeFormLabel.textContent=f?f.title:'尚未選擇問卷';if($('formCount'))$('formCount').textContent=list.length;if($('currentFormName'))$('currentFormName').textContent=f?.title||'—';if($('responseCount'))$('responseCount').textContent=responses.length;renderDashboard();renderFormsTable();if(isSystemAdmin&&$('membersPanel')?.classList.contains('active'))renderMemberPanel();if(isSystemAdmin&&$('trashPanel')?.classList.contains('active'))renderTrash();if($('permissionsPanel')?.classList.contains('active'))loadFormManagers();if($('resultsPanel')?.classList.contains('active'))renderResults();if($('progressPanel')?.classList.contains('active'))renderProgressPanelV171();updateRoleUi()}
...
function resetProgressViewV178(){let filter=$('missingDepartmentFilter'),search=$('missingSearchV179');if(filter)filter.value='';if(search)search.value='';let caption=$('progressCaptionV171'),form=activeForm();if(caption)caption.textContent=form?(form.title+'：正在讀取填寫追蹤…'):'正在讀取填寫追蹤…';if($('completionStats'))$('completionStats').innerHTML='';if($('missingMembersTable'))$('missingMembersTable').innerHTML='<p class="questionHelp loadingBox">正在讀取本問卷的填寫資料…</p>'}
async function switchActiveFormV178(id){let formId=String(id||''),selectionId=++formSelectionRequestV178;if(!formId||!canViewForm(formId))return false;stopResponseListenerV179();activeFormId=formId;history.replaceState(null,'','#manage/'+encodeURIComponent(formId));resetProgressViewV178();if(activeFormSelect)activeFormSelect.setAttribute('aria-busy','true');let loaded=await loadResponses(formId);if(selectionId!==formSelectionRequestV178||activeFormId!==formId){if(activeFormSelect&&selectionId===formSelectionRequestV178)activeFormSelect.removeAttribute('aria-busy');return false}startResponseListenerV179(formId);renderAdmin();if(activeFormSelect)activeFormSelect.removeAttribute('aria-busy');return loaded}
async function selectForm(id){return switchActiveFormV178(id)}
...
async function copyAdminLink(id=activeFormId){if(!id)return notify('請先選擇問卷');let url=location.href.split('#')[0]+'#manage/'+encodeURIComponent(id);if(navigator.clipboard?.writeText)try{await navigator.clipboard.writeText(url);toast('專屬管理網址已複製','success');return}catch(e){}await showCopyDialog('複製專屬管理網址',url)}
...
var FORM_THEMES_V132=[
  {id:'appleWhite',label:'企業簡約',a:'#ffffff',b:'#f5f8fb',accent:'#245f9e'},
  {id:'oceanBlue',label:'清新藍綠',a:'#155e75',b:'#e5f7f6',accent:'#168a87'},
  ...
  {id:'paperWarm',label:'紙張暖白',a:'#6b543d',b:'#faf6ed',accent:'#916b42'},
  {id:'techGradient',label:'科技漸層',a:'#3154a4',b:'#edf2ff',accent:'#5856c8'},
  {id:'freshIllustration',label:'清新插畫',a:'#237c79',b:'#eff9f6',accent:'#df7969'},
  {id:'brandCustom',label:'自訂品牌',a:'#245f9e',b:'#f3f6fa',accent:'#245f9e'}
];
var QUESTION_TYPES_V132=[['short','簡答'],['long','長文'],['single','單選'],['multiple','複選'],['dropdown','下拉選單'],['department','部門選單'],['linearScale','線性刻度'],['rating','星等評分'],['time','時間'],['datetime','日期與時間'],['matrixSingle','單選矩陣'],['matrixMultiple','複選矩陣'],['file','檔案上傳題']];
...
function normalizeQuestion(q){q=q||{};var next={id:q.id||('q_'+Date.now()+'_'+Math.random().toString(36).slice(2,6)),type:q.type||'short',title:q.title||'',description:questionDescription(q),help:questionDescription(q),required:!!q.required,options:Array.isArray(q.options)?q.options:[],rows:Array.isArray(q.rows)?q.rows:[],columns:Array.isArray(q.columns)?q.columns:[],imageUrl:q.imageUrl||'',imageStoragePath:q.imageStoragePath||'',contentFiles:Array.isArray(q.contentFiles)?q.contentFiles.filter(function(file){return file&&file.path}).map(function(file){return{name:String(file.name||'檔案'),path:String(file.path),size:Number(file.size||0),type:String(file.type||'application/octet-stream')}}):[],settings:Object.assign({},q.settings||{}),validation:Object.assign({},q.validation||{}),visibility:normalizeQuestionVisibilityV171(q.visibility)};if(next.type==='image')next.required=false;if(next.type==='linearScale')next.settings={min:Number(next.settings.min!=null?next.settings.min:1),max:Number(next.settings.max!=null?next.settings.max:5),minLabel:next.settings.minLabel||'',maxLabel:next.settings.maxLabel||''};if(next.type==='rating')next.settings={max:Number(next.settings.max||5),minLabel:next.settings.minLabel||'',maxLabel:next.settings.maxLabel||''};if(next.type==='file')next.settings={fileKind:['all','image','document'].includes(next.settings.fileKind)?next.settings.fileKind:'all',multiple:next.settings.multiple===true,maxFiles:Math.max(1,Math.min(5,Number(next.settings.maxFiles||1)))};if(MATRIX_TYPES_V132.includes(next.type)){if(!next.rows.length)next.rows=['項目一'];if(!next.columns.length)next.columns=['選項一']}return next}
...
var activeEditorTabV181='content';
function setEditorTabV181(name){var valid=['content','settings','appearance'];activeEditorTabV181=valid.includes(name)?name:'content';valid.forEach(function(key){var suffix=key.charAt(0).toUpperCase()+key.slice(1),tab=$('editorTab'+suffix+'V181'),pane=$('editorPane'+suffix+'V181'),active=key===activeEditorTabV181;if(tab){tab.classList.toggle('active',active);tab.setAttribute('aria-selected',String(active))}if(pane){pane.classList.toggle('active',active);pane.hidden=!active}})}
function editorDraftFormV181(){return{state:($('formState')||{}).value||'draft',startTime:($('formStartTime')||{}).value||'',deadline:($('formDeadline')||{}).value||''}}
function renderEditorStateV181(){var state=effectiveState(editorDraftFormV181()),badge=$('editorStateBadgeV181'),button=$('editorStateActionV181');if(badge){badge.textContent=stateLabel(state);badge.className='editorStateBadgeV181 state-'+state}if(button){var stored=($('formState')||{}).value||'draft',open=stored==='open';button.textContent=open?'關閉問卷':'開放問卷';button.classList.toggle('danger',open)}}
function toggleEditorStateV181(){var select=$('formState');if(!select)return;select.value=select.value==='open'?'closed':'open';markFormDirty();renderEditorStateV181();toast(select.value==='open'?'已設定為開放，儲存後生效':'已設定為關閉，儲存後生效','success')}
function resetEditorViewV181(){setEditorTabV181('content');var attachments=$('editorAttachmentsV181');if(attachments)attachments.open=false;renderEditorStateV181()}
function startNewForm(){resetPendingImages();resetReferenceFilesV156([]);setDescriptionEditorV156({});formDirty=false;editMode='new';editingId='';draftQuestions=[];$('editorHeading').textContent='新增問卷';$('editorMode').textContent='新增模式';$('saveFormBtn').textContent='建立問卷';$('formTitle').value='';$('formStartTime').value='';$('formDeadline').value='';$('formState').value='draft';$('formImageUrl').value='';setHeaderImageSourceMode('upload');previewHeaderImage();$('identityMode').value='member';renderThemeChoices('appleWhite');renderTargetDepartments([]);renderQuestionEditor();resetEditorViewV181()}
function editForm(id){var f=forms.find(function(x){return x.id===id});if(!f)return;resetPendingImages();resetReferenceFilesV156(f.referenceFiles||[]);setDescriptionEditorV156(f);formDirty=false;editMode='edit';editingId=id;draftQuestions=normalizeQuestions(JSON.parse(JSON.stringify(f.questions||[])));$('editorHeading').textContent='編輯問卷：'+f.title;$('editorMode').textContent='編輯模式';$('saveFormBtn').textContent='儲存變更';$('formTitle').value=f.title||'';$('formStartTime').value=(f.startTime||'').slice(0,16);$('formDeadline').value=(f.deadline||'').slice(0,16);$('formState').value=f.state||'draft';$('formImageUrl').value=f.imageUrl||'';setHeaderImageSourceMode(f.imageStoragePath||!f.imageUrl?'upload':'url');previewHeaderImage();$('identityMode').value=f.identityMode||'none';renderThemeChoices(formTheme(f));renderTargetDepartments(f.targetDepartments||[]);renderQuestionEditor();resetEditorViewV181();showPanel('editorPanel')}
function formQuestionsValid(){var questions=normalizeQuestions(draftQuestions);for(var index=0;index<questions.length;index++){var q=questions[index];if(q.type==='image'){var pending=(pendingContentBlockFilesV175.get(String(q.id))||[]).length,hasContent=!!(q.title.trim()||questionDescription(q).trim()||q.imageUrl||q.imageStoragePath||pendingQuestionImageFiles.get(q.id)||(q.contentFiles||[]).length||pending);if(!hasContent)return'說明內容至少需要標題、說明、圖片或附件其中一項'}else if(!q.title.trim())return'每一題都需要題目名稱';if(['single','multiple','dropdown'].includes(q.type)&&!(q.options||[]).length)return'選擇題至少需要一個選項';if(MATRIX_TYPES_V132.includes(q.type)&&(!(q.rows||[]).length||!(q.columns||[]).length))return'矩陣題至少需要列項目與欄選項';if(q.type==='linearScale'&&Number(q.settings.max)<=Number(q.settings.min))return'線性刻度的結束數字必須大於起始數字';if(q.visibility.enabled){var sourceIndex=questions.findIndex(function(item){return item.id===q.visibility.sourceQuestionId}),source=questions[sourceIndex];if(sourceIndex<0||sourceIndex>=index||!['single','multiple','dropdown','department'].includes((source||{}).type))return'「'+(q.title||'說明內容')+'」的顯示條件必須選擇前面的選項題';if(!q.visibility.values.length)return'「'+(q.title||'說明內容')+'」的顯示條件至少需要選擇一個答案'}}return''}
...
function conditionalEditorHtmlV171(q,i){var visibility=normalizeQuestionVisibilityV171(q.visibility),eligible=conditionalSourceQuestionsV171(i),isContent=normalizeQuestion(q).type==='image',targetName=isContent?'此說明內容':'本題';if(!eligible.length)return '<div class="conditionalBoxV171"><b>條件式顯示</b><p class="questionHelp">前面尚無單選、複選、下拉或部門題，'+targetName+'目前只能永遠顯示。</p></div>';var source=eligible.find(function(item){return item.id===visibility.sourceQuestionId})||eligible[eligible.length-1],options=conditionalSourceOptionsV171(source),selected=new Set(visibility.values.map(String));return '<div class="conditionalBoxV171"><div class="conditionalHeadingV171"><b>條件式顯示</b><span>只有符合條件時才顯示'+targetName+'；不符合時不顯示，也不處理相關答案。</span></div><div class="questionGrid"><label>顯示方式<select onchange="setQuestionVisibilityEnabledV171('+i+',this.value===\'conditional\')"><option value="always" '+(!visibility.enabled?'selected':'')+'>永遠顯示</option><option value="conditional" '+(visibility.enabled?'selected':'')+'>符合條件時顯示</option></select></label>'+(visibility.enabled?'<label>依據題目<select onchange="setQuestionVisibilitySourceV171('+i+',this.value)">'+eligible.map(function(item,index){return '<option value="'+attr(item.id)+'" '+(item.id===source.id?'selected':'')+'>'+(index+1)+'．'+esc(item.title||'未命名題目')+'</option>'}).join('')+'</select></label><label>判斷方式<select onchange="setQuestionVisibilityOperatorV171('+i+',this.value)"><option value="isAnyOf" '+(visibility.operator==='isAnyOf'?'selected':'')+'>回答為任一選項</option><option value="isNotAnyOf" '+(visibility.operator==='isNotAnyOf'?'selected':'')+'>回答不是這些選項</option></select></label>':'')+'</div>'+(visibility.enabled?'<fieldset class="conditionalValuesV171"><legend>觸發答案</legend>'+options.map(function(option,optionIndex){return '<label><input type="checkbox" '+(selected.has(String(option))?'checked':'')+' onchange="toggleQuestionVisibilityValueByIndexV171('+i+','+optionIndex+',this.checked)"><span>'+esc(option)+'</span></label>'}).join('')+'</fieldset>':'')+'</div>'}
function questionMoreBar(q,i){q=normalizeQuestion(q);var isContent=q.type==='image',descOpen=questionSettingOpen('description',i,q),validOpen=questionSettingOpen('validation',i,q),visibilityOpen=questionSettingOpen('visibility',i,q),hasDesc=!!questionDescription(q),hasValid=!isContent&&!!((q.validation||{}).type),hasVisibility=!!((q.visibility||{}).enabled);return '<div class="questionMoreBar"><button class="settingToggle '+(descOpen?'active':'')+'" type="button" onclick="toggleQuestionSetting(\'description\','+i+')">'+(descOpen?'收合說明文字':(isContent?'＋ 說明文字':'＋ 題目說明'))+'</button>'+(!isContent?'<button class="settingToggle '+(validOpen?'active':'')+'" type="button" onclick="toggleQuestionSetting(\'validation\','+i+')">'+(validOpen?'收合回答驗證':'＋ 回答驗證')+'</button>':'')+(i>0?'<button class="settingToggle '+(visibilityOpen?'active':'')+'" type="button" onclick="toggleQuestionSetting(\'visibility\','+i+')">'+(visibilityOpen?'收合顯示條件':'＋ 顯示條件')+'</button>':'')+(hasDesc?settingStatusLabel('已設定說明',true):'')+(hasValid?settingStatusLabel('已設定驗證',true):'')+(hasVisibility?settingStatusLabel('條件式顯示',true):'')+'</div>'}
function questionExtraSettingsHtml(q,i){q=normalizeQuestion(q);var isContent=q.type==='image',descOpen=questionSettingOpen('description',i,q),validOpen=questionSettingOpen('validation',i,q),visibilityOpen=questionSettingOpen('visibility',i,q);return '<div class="collapsibleSetting '+(descOpen?'open':'')+'"><label>'+(isContent?'說明文字':'題目說明')+'<input value="'+attr(questionDescription(q))+'" oninput="updateQuestion('+i+',\'description\',this.value)"></label></div>'+(!isContent?'<div class="collapsibleSetting '+(validOpen?'open':'')+'">'+validationEditorHtml(q,i)+'</div>':'')+(i>0?'<div class="collapsibleSetting '+(visibilityOpen?'open':'')+'">'+conditionalEditorHtmlV171(q,i)+'</div>':'')}
function questionImageEditorHtml(q,i){q=normalizeQuestion(q);var pending=pendingQuestionImageFiles.get(q.id),preview=pendingQuestionImagePreviewUrls.get(q.id)||imageUrl(q.imageUrl),urlMode=!pending&&!q.imageStoragePath&&!!q.imageUrl,status=pending?'待上傳至 Firebase Storage':(q.imageStoragePath?'目前使用 Firebase Storage 圖片':(q.imageUrl?'目前使用網址圖片':'尚未選擇圖片')),owner=q.type==='image'?'說明內容':'題目';return '<div class="questionImageField imageSourceField"><b>圖片上傳</b><div class="imageSourceTabs" role="group" aria-label="'+owner+'圖片來源"><button id="questionImageUploadTab_'+i+'" class="imageSourceTab '+(!urlMode?'active':'')+'" type="button" onclick="setQuestionImageSourceMode('+i+',\'upload\')">上傳圖片</button><button id="questionImageUrlTab_'+i+'" class="imageSourceTab '+(urlMode?'active':'')+'" type="button" onclick="setQuestionImageSourceMode('+i+',\'url\')">貼上網址</button></div><div id="questionImageUploadPanel_'+i+'" class="imageSourcePanel" '+(urlMode?'hidden':'')+'><input id="questionImageFile_'+i+'" class="hiddenFileInput" type="file" accept="image/jpeg,image/png,image/webp" onchange="handleQuestionImageFile('+i+',this.files[0]);this.value=\'\'" aria-label="上傳'+owner+'圖片"><div class="imageUploadRow"><button class="btn" type="button" onclick="chooseQuestionImageFile('+i+')">選擇圖片</button><span class="imageFileName">'+esc(pending?pending.name:'JPG、PNG、WebP')+'</span></div></div><div id="questionImageUrlPanel_'+i+'" class="imageSourcePanel" '+(!urlMode?'hidden':'')+'><label>圖片網址<input value="'+attr(q.imageUrl||'')+'" placeholder="貼上 Google Drive 或一般圖片網址" oninput="updateQuestionImage('+i+',this.value)"></label><p>Drive 圖片需開啟「知道連結的使用者皆可查看」。</p></div><div class="imagePreviewToolbar"><span class="imageSourceStatus">'+esc(status)+'</span>'+(preview?'<button class="btn danger" type="button" onclick="clearQuestionImage('+i+')">移除圖片</button>':'')+'</div><div id="questionImagePreview_'+i+'" class="imagePreview">'+previewImageHtmlFromUrl(preview,q.title||'圖片預覽')+'</div></div>'}
...
function copyQuestion(i){var source=normalizeQuestion(draftQuestions[i]),copy=JSON.parse(JSON.stringify(source));copy.id='q_'+Date.now()+'_'+Math.random().toString(36).slice(2,6);copy.imageStoragePath='';copy.contentFiles=[];copy.title=source.type==='image'?(copy.title?copy.title+'（複製）':''):(copy.title||'未命名題目')+'（複製）';draftQuestions.splice(i+1,0,copy);window.__scrollToQuestionIndex=i+1;renderQuestionEditor();toast(source.type==='image'?'說明內容已複製；附件請重新選擇':'題目已複製','success')}
...
function frontTopHtml(f,state){var statusLabel=f?stateLabel(state):'請使用完整網址';return '<header class="frontHeader"><img src="assets/company-logo.png" alt="環興科技股份有限公司"><div class="frontActions"><span id="formStatus" class="statusPill">'+esc(statusLabel)+'</span>'+(isAdmin?'':'<button class="ghostBtn" onclick="openAdmin()">管理登入</button>')+'</div></header>'}
function frontUnavailableHtmlV181(f,state){if(state==='scheduled')return '<div class="successCard frontScheduleNoticeV181" style="margin-top:18px"><h2>問卷尚未開放</h2><p>預計於 '+esc(formatDeadline(f.startTime))+' 開放填寫。</p></div>';if(state==='expired')return '<div class="successCard" style="margin-top:18px"><h2>本問卷已截止填寫</h2></div>';return '<div class="successCard" style="margin-top:18px"><h2>本問卷目前未開放填寫</h2></div>'}
function renderFront(){var route=formRouteId(),publicForms=forms.filter(function(x){return x.deleted!==true});if(!route){activeFormId='';document.body.removeAttribute('data-front-theme');document.body.classList.add('homeLoginActiveV176');formStatus.textContent='管理登入';frontMain.innerHTML=homeLoginHtmlV176();return}document.body.classList.remove('homeLoginActiveV176');var f=publicForms.find(function(x){return x.id===route});if(!f){activeFormId='';document.body.removeAttribute('data-front-theme');formStatus.textContent='找不到問卷';frontMain.innerHTML=frontPreviewBannerHtml()+frontTopHtml(null,'closed')+'<div class="successCard"><h2>找不到這份問卷</h2><p>請確認問卷網址是否完整，或洽問卷管理者重新提供連結。</p></div>';return}activeFormId=f.id;applyFormTheme(f);var state=effectiveState(f),available=state==='open',heroImage=imageUrl(f.imageUrl),descriptionHtml=frontDescriptionHtmlV156(f),referenceFiles=frontReferenceFilesHtmlV156(f);formStatus.textContent=stateLabel(state);frontMain.innerHTML=frontPreviewBannerHtml()+frontTopHtml(f,state)+'<section class="formHero"><h1>'+esc(f.title)+'</h1>'+descriptionHtml+(heroImage?'<img class="referenceImage" src="'+attr(heroImage)+'" alt="問卷參考圖片">':'')+referenceFiles+(f.deadline?'<span class="deadlineBadge">請於 '+esc(formatDeadline(f.deadline))+' 前完成</span>':'')+'</section>'+(!available?frontUnavailableHtmlV181(f,state):'<form id="publicForm" class="questionList frontFormStack" onsubmit="submitResponse(event)"><div class="frontFormHeading"><h2>填寫問卷</h2></div>'+(f.identityMode==='member'?renderIdentityBlock(f):'')+normalizeQuestions(f.questions||[]).map(function(q){return renderPublicQuestion(q)}).join('')+'<div id="submitNotice" class="frontSubmitNotice questionHelp" hidden aria-live="polite"></div><div class="submitArea"><button id="submitBtn" class="btn primary" type="submit">確認並送出</button></div></form>')}
...
function renderMissingMembers(form){var panel=ensureMissingPanel();if(!panel)return;if(!form||form.identityMode!=='member'){panel.style.display='none';return}panel.style.display='block';var data=completionData(form),filter=$('missingDepartmentFilter'),previous=filter.value,deps=Array.from(new Set(data.expected.map(memberDepartmentName).filter(Boolean))).sort(function(a,b){return a.localeCompare(b,'zh-Hant')});filter.innerHTML='<option value="">全部部門</option>'+deps.map(function(dep){return '<option value="'+attr(dep)+'">'+esc(dep)+'</option>'}).join('');if(previous&&deps.includes(previous))filter.value=previous;var selected=filter.value,scoped=missingScopeData(form,selected),keyword=String(($('missingSearchV179')||{}).value||'').trim().toLowerCase(),list=scoped.missing.filter(function(member){return !keyword||[memberDepartmentName(member),member.name,memberEmployeeNo(member)].join(' ').toLowerCase().includes(keyword)}),manage=canManageForm(form.id);$('missingCaption').textContent=(selected?selected+'：':'全部部門：')+'顯示 '+list.length+' 位未填寫人員'+(keyword?'（搜尋結果）':'');$('completionStats').innerHTML='<div class="completionStat"><span>應填人數</span><strong>'+scoped.expected.length+' 人</strong></div><div class="completionStat"><span>已填人數</span><strong>'+scoped.filled.length+' 人</strong></div><div class="completionStat"><span>未填人數</span><strong>'+scoped.missing.length+' 人</strong></div>';$('missingMembersTable').innerHTML=list.length?table(['部門','姓名','員工編號','狀態'].concat(manage?['操作']:[]),list.map(function(member){return '<tr><td>'+esc(memberDepartmentName(member))+'</td><td><b>'+esc(member.name||'')+'</b></td><td>'+esc(memberEmployeeNo(member))+'</td><td><span class="statePill state-draft">未填寫</span></td>'+(manage?'<td>'+actionGroup([actionButton('協助填寫',"openAssistedFill('"+attr(member.id)+"')")])+'</td>':'')+'</tr>'})):emptyState(selected||keyword?'目前篩選條件沒有未填寫人員':'所有應填人員皆已完成','請調整搜尋或部門篩選條件。')}
...
async function submitResponse(event){
  ...
  var f=activeForm();
  if(!f||effectiveState(f)!=='open')return notify(effectiveState(f)==='scheduled'?'問卷尚未開放，請於開放時間後再填寫':'問卷已關閉，請重新整理頁面');
...
function questionJumpSelectHtmlV140(index){
  ...
  return '<label class="questionJumpControlV140">移至位置<select onchange="moveQuestionToV140('+index+',Number(this.value)-1)">'+draftQuestions.map(function(_,idx){return '<option value="'+(idx+1)+'" '+(idx===index?'selected':'')+'>'+(idx+1)+'</option>'}).join('')+'</select></label>';
}
...
function renderQuestionEditor(){
  ...
  var html=draftQuestions.map(function(q,i){
    var isContent=q.type==='image',questionNumber=draftQuestions.slice(0,i+1).filter(function(item){return normalizeQuestion(item).type!=='image'}).length,titleField='<label>'+(isContent?'說明標題（選填）':'題目名稱')+'<input value="'+attr(q.title)+'" oninput="updateQuestion('+i+',\'title\',this.value)"></label>',typeField=isContent?'':'<label>題型<select onchange="updateQuestion('+i+',\'type\',this.value);renderQuestionEditor()">'+QUESTION_TYPES_V132.map(function(x){return '<option value="'+x[0]+'" '+(q.type===x[0]?'selected':'')+'>'+x[1]+'</option>'}).join('')+'</select></label>',requiredField=isContent?'':'<label>必填<select onchange="updateQuestion('+i+',\'required\',this.value===\'true\')"><option value="false" '+(!q.required?'selected':'')+'>否</option><option value="true" '+(q.required?'selected':'')+'>是</option></select></label>',purpose=isContent?'<span class="contentPurposeBadgeV180">不需作答</span>':'';
    return '<div class="questionEdit '+(isContent?'contentBlockEditorV175 contentBlockEditorV180':'answerQuestionEditorV180')+'" data-question-index="'+i+'" draggable="true" ondragstart="onQuestionDragStart(event,'+i+')" ondragover="onQuestionDragOver(event,'+i+')" ondragleave="onQuestionDragLeave(event)" ondrop="onQuestionDrop(event,'+i+')" ondragend="onQuestionDragEnd(event)"><div class="questionEditHeader"><button type="button" class="dragHandle" title="拖曳排序" aria-label="拖曳排序">⋮⋮</button><span class="questionNumber">'+(isContent?'說明內容':'題目 '+questionNumber)+'</span>'+purpose+questionJumpSelectHtmlV140(i)+'</div>'+(isContent?'<p class="contentPurposeHelpV180">插入文字、圖片或附件供填寫者閱讀，不會產生作答欄位，也不列入題號。</p>':'')+'<div class="questionGrid '+(isContent?'contentBlockMainGridV175 contentBlockMainGridV180':'')+'">'+titleField+typeField+requiredField+'</div>'+optionEditorHtml(q,i)+questionImageEditorHtml(q,i)+contentBlockFileEditorHtmlV175(q,i)+questionMoreBar(q,i)+questionExtraSettingsHtml(q,i)+'<div class="miniActions"><button class="btn" type="button" onclick="moveQuestion('+i+',-1)">上移</button><button class="btn" type="button" onclick="moveQuestion('+i+',1)">下移</button><button class="btn" type="button" onclick="copyQuestion('+i+')">'+(isContent?'複製說明內容':'複製題目')+'</button><button class="btn danger" type="button" onclick="removeQuestion('+i+')">移除</button></div></div>';
  }).join('');
  questionEditor.innerHTML=html||'<div class="questionHelp questionEmptyHelpV180">尚未建立作答題目；如需補充說明，也可先加入說明內容。</div>';
  questionEditor.insertAdjacentHTML('beforeend','<div class="questionAddBottom questionAddBottomV180"><div class="questionAddChoiceV180 answerChoiceV180"><button class="btn primary" type="button" onclick="addQuestion()">新增作答題目</button><small>建立需要填寫者回答的題目</small></div><div class="questionAddChoiceV180 contentChoiceV180"><button class="btn" type="button" onclick="addQuestion(\'image\')">加入說明內容</button><small>插入文字、圖片或附件，不需作答</small></div></div>');
...
async function duplicateForm(id){
  ...
  var targetIds=formTargetMemberIdsV140(source),newId='form_'+Date.now(),copiedQuestions=normalizeQuestions(JSON.parse(JSON.stringify(source.questions||[])));copiedQuestions.forEach(function(q){q.imageStoragePath='';q.contentFiles=[]});var data={title:(source.title||'未命名問卷')+'（複製）',description:source.description||'',descriptionHtml:source.descriptionHtml||'',descriptionFontSize:normalizeDescriptionFontSizeV156(source.descriptionFontSize),descriptionAlign:normalizeDescriptionAlignV156(source.descriptionAlign),deadline:'',state:'draft',imageUrl:source.imageUrl||'',imageStoragePath:'',referenceFiles:[],theme:formTheme(source),heroStyle:['compact','banner','image','sideAccent'].includes(source.heroStyle)?source.heroStyle:'banner',customTheme:source.customTheme&&typeof source.customTheme==='object'?Object.assign({},source.customTheme):{},fillLayout:source.fillLayout==='focus'?'focus':'standard',completionMessage:source.completionMessage||'',showResponseSummary:source.showResponseSummary!==false,allowResponsePrint:source.allowResponsePrint!==false,schemaVersion:4,identityMode:source.identityMode||'member',targetDepartments:[].concat(source.targetDepartments||[]),questions:copiedQuestions,createdByUid:(currentUser&&currentUser.uid)||'',createdByEmail:normalizeEmail((currentUser&&currentUser.email)||''),createdByName:adminDisplayName()||(currentUser&&currentUser.displayName)||(currentUser&&currentUser.email)||'',createdAt:firebase.firestore.FieldValue.serverTimestamp(),updatedAt:firebase.firestore.FieldValue.serverTimestamp(),updatedByUid:(currentUser&&currentUser.uid)||'',updatedByEmail:normalizeEmail((currentUser&&currentUser.email)||''),updatedByName:adminDisplayName()};
...
function renderDashboard(){
  var f=activeForm(),memberMode=formUsesMemberDatabaseV141(f),progress=memberMode?completionData(f):null,rate=progress&&progress.expected.length?Math.round(progress.filled.length*100/progress.expected.length):null,questionCount=f?normalizeQuestions(f.questions||[]).filter(function(q){return q.type!=='image'}).length:0;
...
async function prepareStoredImagesV153(formId,existing,questions){
  ...
  try{
    ...
    for(var q of questions){
      var old=oldQuestions.get(String(q.id)),oldPath=(old&&old.imageStoragePath)||'',oldUrl=(old&&old.imageUrl)||'',pending=pendingQuestionImageFiles.get(q.id);
      if(pending){var image=await uploadImageFileV153(formId,'question',q.id,pending);uploaded.push(image.path);q.imageUrl=image.url;q.imageStoragePath=image.path}else if(oldPath&&q.imageUrl===oldUrl)q.imageStoragePath=oldPath;else q.imageStoragePath=q.imageStoragePath||'';
      if(oldPath&&oldPath!==q.imageStoragePath)obsolete.push(oldPath);
      var oldContentFiles=[].concat((old&&old.contentFiles)||[]).filter(function(file){return file&&file.path}),retainedContentFiles=[].concat(q.contentFiles||[]).filter(function(file){return file&&file.path}),retainedContentPaths=new Set(retainedContentFiles.map(function(file){return file.path})),pendingContentFiles=pendingContentBlockFilesV175.get(String(q.id))||[],savedContentFiles=[];
      for(var contentItem of pendingContentFiles){var savedContent=await uploadContentBlockFileV175(formId,q.id,contentItem);uploaded.push(savedContent.path);savedContentFiles.push(savedContent)}
      q.contentFiles=retainedContentFiles.concat(savedContentFiles);
      oldContentFiles.forEach(function(file){if(!retainedContentPaths.has(file.path))obsolete.push(file.path)});
      oldQuestions.delete(String(q.id));
    }
    oldQuestions.forEach(function(q){if(q.imageStoragePath)obsolete.push(q.imageStoragePath);[].concat(q.contentFiles||[]).forEach(function(file){if(file&&file.path)obsolete.push(file.path)})});
...
function resetPendingContentBlockFilesV175(){pendingContentBlockFilesV175.clear()}
function contentBlockFileKeyV175(file){return String((file&&file.path)||((file&&file.pendingId)?'pending:'+file.pendingId:''))}
function chooseContentBlockFilesV175(index){var input=$('contentBlockFilesV175_'+index);if(input)input.click()}
function handleContentBlockFilesV175(index,fileList){
  var q=normalizeQuestion(draftQuestions[index]),files=Array.from(fileList||[]);if(q.type!=='image'||!files.length)return;
  var current=pendingContentBlockFilesV175.get(String(q.id))||[];
  files.forEach(function(file){current.push({file:file,pendingId:Date.now()+'_'+Math.random().toString(36).slice(2,8)})});
  pendingContentBlockFilesV175.set(String(q.id),current);markFormDirty();renderQuestionEditor()
}
function removeContentBlockFileV175(index,key){
  var q=draftQuestions[index]=normalizeQuestion(draftQuestions[index]),id=String(q.id);
  q.contentFiles=(q.contentFiles||[]).filter(function(file){return contentBlockFileKeyV175(file)!==key});
  pendingContentBlockFilesV175.set(id,(pendingContentBlockFilesV175.get(id)||[]).filter(function(item){return contentBlockFileKeyV175(item)!==key}));
  markFormDirty();renderQuestionEditor()
}
function contentBlockFileEditorHtmlV175(q,index){
  q=normalizeQuestion(q);if(q.type!=='image')return '';
  var existing=(q.contentFiles||[]).map(function(file){return{key:contentBlockFileKeyV175(file),name:file.name||'檔案',pending:false}}),pending=(pendingContentBlockFilesV175.get(String(q.id))||[]).map(function(item){return{key:contentBlockFileKeyV175(item),name:item.file.name,pending:true}}),all=existing.concat(pending);
  return '<div class="contentBlockFileFieldV175"><div class="contentBlockFileHeadingV175"><div><b>附件</b><small>可加入 PDF、Word、Excel 或其他檔案；不設定系統自訂容量及數量上限。</small></div><button class="btn" type="button" onclick="chooseContentBlockFilesV175('+index+')">選擇檔案</button></div><input id="contentBlockFilesV175_'+index+'" class="hiddenFileInput" type="file" multiple onchange="handleContentBlockFilesV175('+index+',this.files);this.value=\'\'" aria-label="上傳說明內容附件"><div class="contentBlockFileListV175">'+(all.length?all.map(function(file){return '<div class="contentBlockFileItemV175">'+paperclipIconV173()+'<span title="'+attr(file.name)+'">'+esc(file.name)+'</span>'+(file.pending?'<small>儲存問卷時上傳</small>':'')+'<button class="btn danger" type="button" onclick="removeContentBlockFileV175('+index+',\''+attr(file.key)+'\')">移除</button></div>'}).join(''):'<p>尚未加入附件</p>')+'</div></div>'
}
async function uploadContentBlockFileV175(formId,questionId,item){
  if(!storage)throw new Error('Firebase Storage 尚未初始化');
  var file=item.file,fileName=Date.now()+'_'+Math.random().toString(36).slice(2,8)+'_'+storageSafeSegmentV153(file.name),path='universal-forms/'+storageSafeSegmentV153(formId)+'/references/content-'+storageSafeSegmentV153(questionId)+'/'+fileName,ref=storage.ref().child(path);
  await ref.put(file,{contentType:file.type||'application/octet-stream',customMetadata:{formId:String(formId),questionId:String(questionId),kind:'content-block',originalName:file.name,uploadedByUid:(currentUser&&currentUser.uid)||''}});
  return{name:file.name,path:path,size:file.size,type:file.type||'application/octet-stream'}
}
function frontContentBlockFilesHtmlV175(q){
  var files=[].concat((q&&q.contentFiles)||[]).filter(function(file){return file&&file.path});if(!files.length)return '';
  return '<div class="frontAttachmentsV174 contentBlockAttachmentsV175"><div>'+files.map(function(file){var name=String(file.name||'檔案');return '<button type="button" title="'+attr('開啟附件：'+name)+'" aria-label="'+attr('開啟附件：'+name)+'" onclick="openReferenceFileV156(\''+attr(file.path)+'\')">'+frontPaperclipIconV174()+'<span>'+esc(name)+'</span></button>'}).join('')+'</div></div>'
}
function renderReferenceFilesV156(){
  ...
  if($('editorAttachmentCountV181'))$('editorAttachmentCountV181').textContent=String(all.length);
...
async function saveForm(){
  ...
  var startTime=$('formStartTime').value,deadline=$('formDeadline').value;
  if(startTime&&deadline&&new Date(startTime).getTime()>new Date(deadline).getTime()){setEditorTabV181('settings');return notify('開啟問卷時間不可晚於截止時間')}
  ...
  var id=editMode==='edit'?editingId:'form_'+Date.now(),existing=editMode==='edit'?forms.find(function(f){return f.id===editingId}):null,descriptionData=descriptionDataV156(),data={title:title,description:descriptionData.description,descriptionHtml:descriptionData.descriptionHtml,descriptionFontSize:descriptionData.descriptionFontSize,descriptionAlign:descriptionData.descriptionAlign,startTime:startTime,deadline:deadline,state:$('formState').value,imageUrl:$('formImageUrl').value.trim(),imageStoragePath:'',theme:formTheme({theme:($('formTheme')||{}).value}),heroStyle:typeof heroStyleDataV188==='function'?heroStyleDataV188():'banner',customTheme:typeof customThemeDataV188==='function'?customThemeDataV188():{},fillLayout:(($('formFillLayoutV187')||{}).value==='focus'?'focus':'standard'),completionMessage:($('completionMessageV185')||{}).value||'',showResponseSummary:$('showResponseSummaryV185')?$('showResponseSummaryV185').checked:true,allowResponsePrint:$('allowResponsePrintV185')?$('allowResponsePrintV185').checked:true,schemaVersion:4,identityMode:identityMode,targetDepartments:targetDepartments,questions:draftQuestions,updatedAt:firebase.firestore.FieldValue.serverTimestamp(),updatedByUid:(currentUser&&currentUser.uid)||'',updatedByEmail:normalizeEmail((currentUser&&currentUser.email)||''),updatedByName:adminDisplayName()};
...
function addQuestionAfterV143(index,type){
  ...
  renderQuestionEditor();
  toast('已在第 '+(index+1)+' 題下方新增作答題目','success');
}
function enhanceQuestionEditorV143(){
  ...
  editor.querySelectorAll('.questionEdit').forEach(function(card){
    ...
    if(actions&&!actions.querySelector('.questionAddAfterV143')){
      actions.insertAdjacentHTML('afterbegin','<button class="btn questionAddAfterV143" type="button" onclick="addQuestionAfterV143('+index+')">下方新增作答題目</button><button class="btn contentAddAfterV175" type="button" onclick="addQuestionAfterV143('+index+',\'image\')">下方加入說明內容</button>');
    }
  });
...
if(updateQuestionV144Base){
  updateQuestion=function(i,key,value){
    ...
    if(key==='type'){
      draftQuestions[i]=cleanQuestionForTypeV144(draftQuestions[i]);
      if(value!=='image'){draftQuestions[i].contentFiles=[];pendingContentBlockFilesV175.delete(String(oldId))}
    }
...
if(removeQuestionV144Base){
  removeQuestion=async function(i){
    i=Number(i);
    var oldId=questionIdAtV144(i),isContent=normalizeQuestion(draftQuestions[i]).type==='image';
    var ok=await confirmDialog(isContent?'確定移除此說明內容？':'確定移除此題？',isContent?'移除說明內容':'移除題目',true);
    if(!ok)return;
    ...
    pendingContentBlockFilesV175.delete(String(oldId));
...
if(addQuestionAfterV143V144Base){
  addQuestionAfterV143=function(index,type){
    ...
    renderQuestionEditor();
    focusQuestionByIdV144(activeQuestionIdV144);
    toast(type==='image'?'已在下方加入說明內容':'已在下方新增作答題目','success');
  };
}
...
deleteForm=async function(id){
  ...
  try{
    ...
    var hasStorage=!!f.imageStoragePath||[].concat(f.referenceFiles||[]).some(function(file){return !!(file&&file.path)})||normalizeQuestions(f.questions||[]).some(function(q){return !!q.imageStoragePath||[].concat(q.contentFiles||[]).some(function(file){return !!(file&&file.path)})});
...
/* v1.75: sortable content blocks can include descriptions, images, and downloadable files. */
var renderPublicQuestionV175Base=renderPublicQuestion;
renderPublicQuestion=function(q,prefix,value){
  q=normalizeQuestion(q);
  if(q.type!=='image')return renderPublicQuestionV175Base(q,prefix,value);
  var conditional=q.visibility.enabled?' hidden aria-hidden="true"':' aria-hidden="false"',img=imageUrl(q.imageUrl),title=q.title?'<h3>'+esc(q.title)+'</h3>':'',description=questionDescription(q)?'<div class="contentBlockDescriptionV175">'+esc(questionDescription(q))+'</div>':'',image=img?'<img class="questionImage" src="'+attr(img)+'" alt="'+attr(q.title||'內容圖片')+'">':'';
  scheduleConditionalRefreshV171();
  return '<section data-question-id-v171="'+attr(q.id)+'"'+conditional+' class="questionCard contentBlockV175">'+title+description+image+frontContentBlockFilesHtmlV175(q)+'</section>'
};
