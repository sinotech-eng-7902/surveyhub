...
async function openMissingList(button){resultViewModeV170='individual';await showPanel('resultsPanel',button);updateResultViewVisibilityV170();let panel=ensureMissingPanel();if(panel)panel.scrollIntoView({behavior:'smooth',block:'start'})}
...
function updateResultViewVisibilityV170(){var map={summary:'resultAnalysis',question:'resultQuestionViewV170',individual:'resultIndividualViewV170'};Object.keys(map).forEach(function(mode){var panel=$(map[mode]);if(panel)panel.hidden=resultViewModeV170!==mode});document.querySelectorAll('[data-result-view-v170]').forEach(function(button){var selected=button.dataset.resultViewV170===resultViewModeV170;button.setAttribute('aria-selected',selected?'true':'false');button.tabIndex=selected?0:-1;button.disabled=!activeForm()})}
function setResultViewV170(mode,button){if(!['summary','question','individual'].includes(mode))return;resultViewModeV170=mode;updateResultViewVisibilityV170();if(button)button.focus()}
...
function renderResults(){
  ...
  if(!f){$('resultAnalysis').innerHTML='';$('resultQuestionViewV170').innerHTML='';$('resultIndividualNavigatorV170').innerHTML='';resultsTable.innerHTML='';var p=$('missingResponsesPanel');if(p)p.style.display='none';updateResultViewVisibilityV170();return}
  ...
  renderIndividualNavigatorV170(f,list);
