...
async function logout(){activeFormId='';loginPurpose='admin';setLoginFeedbackV176('');await auth.signOut();history.replaceState(null,'',location.pathname+location.search);applyRoute()}
...
function resetProgressViewV178(){let filter=$('missingDepartmentFilter');if(filter)filter.value='';let caption=$('progressCaptionV171'),form=activeForm();if(caption)caption.textContent=form?(form.title+'：正在讀取填寫追蹤…'):'正在讀取填寫追蹤…';if($('completionStats'))$('completionStats').innerHTML='';if($('missingMembersTable'))$('missingMembersTable').innerHTML='<p class="questionHelp loadingBox">正在讀取本問卷的填寫資料…</p>'}
async function switchActiveFormV178(id){let formId=String(id||''),selectionId=++formSelectionRequestV178;if(!formId||!canViewForm(formId))return false;activeFormId=formId;history.replaceState(null,'','#manage/'+encodeURIComponent(formId));resetProgressViewV178();if(activeFormSelect)activeFormSelect.setAttribute('aria-busy','true');let loaded=await loadResponses(formId);if(selectionId!==formSelectionRequestV178||activeFormId!==formId){if(activeFormSelect&&selectionId===formSelectionRequestV178)activeFormSelect.removeAttribute('aria-busy');return false}renderAdmin();if(activeFormSelect)activeFormSelect.removeAttribute('aria-busy');return loaded}
